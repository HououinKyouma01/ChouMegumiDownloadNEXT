...
import com.example.megumidownload.PermissionManager
...
@Composable
fun DashboardScreen(viewModel: LogViewModel, permissionManager: PermissionManager) {
    ...
    val downloadManager = remember { DownloadManager(context, configManager, seriesManager) }
    ...
    Box(modifier = Modifier.fillMaxSize()) {
        Column(modifier = Modifier.fillMaxSize().padding(16.dp)) {
            ...
            Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                ...
                Button(onClick = {
                    // Start WorkManager
                    val workRequest = androidx.work.OneTimeWorkRequestBuilder<com.example.megumidownload.DownloadWorker>()
                        .build()
        ...
        LaunchedEffect(autoStart) {
            if (autoStart && logs.isEmpty()) { // Simple check to avoid re-triggering if already ran
                val workRequest = androidx.work.OneTimeWorkRequestBuilder<com.example.megumidownload.DownloadWorker>()
                    .build()
                androidx.work.WorkManager.getInstance(context).enqueueUniqueWork(
                    "DownloadWorker",
