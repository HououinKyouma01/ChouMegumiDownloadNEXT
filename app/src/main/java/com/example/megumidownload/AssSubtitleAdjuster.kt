...
object AssSubtitleAdjuster {
    ...
    fun adjustTiming(inputFile: File, outputFile: File, offsetMs: Long, keyframesMs: List<Long> = emptyList()) {
        ...
        if (keyframesMs.isNotEmpty()) {
            android.util.Log.d("AssSubtitleAdjuster", "Applying keyframe snapping with ${keyframesMs.size} keyframes.")
            applyKeyframeSnapping(events, keyframesMs)
        } else {
            android.util.Log.d("AssSubtitleAdjuster", "No keyframes provided. Applying simple offset only.")
        }
