...
class SyncManager(
    ...
    private fun loadGroups(): List<String> {
        val groupsFile = File(filesDir, "groups.megumi")
        ...
        return try {
            ...
            Logger.e("SyncManager", "Error loading groups.megumi: ${e.message}")
            emptyList()
        }
    }
    ...
    suspend fun syncAll(
        ...
        notificationService.showProgressNotification("Sync Started", "Preparing...", 0, 100, true)
        ...
        try {
            ...
            for (series in seriesList) {
                ...
                notificationService.showProgressNotification("Syncing ${series.folderName}", "Checking files...", (seriesProgressBase * 100).toInt(), 100)
                ...
                val result = syncSeriesInternal(
                    ...
                    onProgress = { msg, fileProgress, fileIndex, fileCount -> 
                        ...
                        notificationService.showProgressNotification("Syncing ${series.folderName}", msg, progressInt, 100)
                    },
                    onConflict = onConflict
                )
                
                if (result is SyncResult.Error) {
                    Logger.e("SyncManager", "Error syncing ${series.folderName}: ${result.message}")
            ...
            Logger.e("SyncManager", "SyncAll error: ${e.message}")
    ...
    suspend fun fetchRemoteFiles(series: SeriesEntry): Result<List<String>> = withContext(Dispatchers.IO) {
        try {
            ...
            if (smbClient.exists(remotePath)) {
                ...
                Logger.d("SyncManager", "fetchRemoteFiles: Path=$remotePath, FilesFound=${files.size}, EnableGroups=$enableGroups, Groups=$groups")
                ...
                Logger.d("SyncManager", "fetchRemoteFiles: Filtered MKVs=${mkvFiles.size}")
                Result.success(mkvFiles)
            } else {
                Logger.w("SyncManager", "fetchRemoteFiles: Remote path does not exist: $remotePath")
             ...
             for ((index, rFile) in filesToSync.withIndex()) {
                ...
                if (rFile.name.endsWith(".mkv")) {
                    ...
                    if (enableGroups && !isFileAllowedByGroups(rFile.name, groups)) {
                        Logger.d("SyncManager", "Skipping ${rFile.name} (Group filter)")
                    ...
                    if (options.selectedFiles != null && !options.selectedFiles.contains(rFile.name)) {
                        Logger.d("SyncManager", "Skipping ${rFile.name} (Not selected)")
                ...
                if (lFile.exists()) {
                    ...
                    val isConflict = if (rFile.name == "replace.txt") {
                        try {
                            val tempFile = File.createTempFile("compare", ".txt", cacheDir)
                            ...
                            Logger.e("SyncManager", "Error comparing replace.txt: ${e.message}")
    ...
    private suspend fun mergeFilelist(localFile: File, remotePath: String) {
        ...
        val tempFile = File.createTempFile("remote_filelist", ".txt", cacheDir)
        try {
            ...
            Logger.e("SyncManager", "Error merging filelist: ${e.message}")
        } finally {
            tempFile.delete()
        }
    }
    ...
    suspend fun readRemoteFileContent(remotePath: String): String = withContext(Dispatchers.IO) {
        val tempFile = File.createTempFile("remote_read", ".tmp", cacheDir)
        try {
            smbClient.downloadFile(remotePath, tempFile)
    ...
    suspend fun writeRemoteFileContent(remotePath: String, content: String) = withContext(Dispatchers.IO) {
        val tempFile = File.createTempFile("remote_write", ".tmp", cacheDir)
        try {
    ...
    suspend fun getRemoteReplaceContent(series: SeriesEntry): Result<String> = withContext(Dispatchers.IO) {
        try {
            ...
            Logger.d("SyncManager", "Downloading replace.txt from $remotePath")
            
            val tempFile = File.createTempFile("remote_replace", ".txt", cacheDir)
            try {
                smbClient.downloadFile(remotePath, tempFile)
                val content = tempFile.readText()
                return@withContext Result.success(content)
            } catch (e: java.io.FileNotFoundException) {
                Logger.w("SyncManager", "Remote replace.txt not found at $remotePath")
                return@withContext Result.success("") // Return empty string if not found
            } catch (e: Exception) {
                Logger.e("SyncManager", "Error downloading replace.txt: ${e.message}")
            ...
            Logger.e("SyncManager", "Error getting remote replace.txt: ${e.message}")
            smbClient.disconnect()
            val stackTrace = e.stackTrace.firstOrNull()
    ...
    suspend fun saveRemoteReplaceContent(series: SeriesEntry, content: String): Boolean = withContext(Dispatchers.IO) {
        try {
            ...
            val tempFile = File.createTempFile("remote_replace_save", ".txt", cacheDir)
            tempFile.writeText(content)
            try {
                smbClient.uploadFile(tempFile, remotePath)
            ...
            Logger.e("SyncManager", "Error saving remote replace.txt: ${e.message}")
            smbClient.disconnect()
    ...
    private suspend fun connectToShare(): String? {
        ...
        try {
            ...
            Logger.e("SyncManager", "Error loading config: ${e.message}")
            return null
        }
        
        if (host.isBlank() || sharePath.isBlank()) {
            Logger.e("SyncManager", "Host or SharePath is blank")
            return null
        }
